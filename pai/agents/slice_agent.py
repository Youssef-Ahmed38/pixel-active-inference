...
Everything above L0 sees only entity tokens, so the same loop later runs on learned slots.
"""
...
def run_episode(env, wm, cfg, episode: int, seed: int, disturbances: list[dict] | None = None,
                ...
                raw: list | None = None) -> EpisodeRecord:
    ...
    for k in range(sc.max_plan_steps):
        ...
        new_tokens = encode(env, obs, prev_ee)
        ...
        if raw is not None:
            raw.append((residual, model_sigma))
        ev = StepEvidence(k, residual, combined_sigma(model_sigma, calibration), a_np,
